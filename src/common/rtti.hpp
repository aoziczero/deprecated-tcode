#ifndef __tcode_util_rtti_h__
#define __tcode_util_rtti_h__
...
public:
	...
	class get_handler {
	public:
		typedef const tcode::rtti* (*handler_function)();

		get_handler( void );
		~get_handler( void );

		const tcode::rtti* operator()( void ) const ;
...
public: 													                \
	tcode::rtti::get_handler	 rtti;						                \
	static const tcode::rtti	 s_rtti;					                \
	static const tcode::rtti* _rtti_impl( void) {			                \
		return &class_name::s_rtti;							                \
	}														                \
	template < typename type >								                \
	bool is_kind_of() {										                \
		const tcode::rtti* r = rtti();						                \
		while ( r != nullptr ){								                \
			if ( r == &type::s_rtti ) {						                \
				return true;								                \
			}												                \
...
public:														                \
	static  const tcode::rtti  s_rtti;						                \
	virtual const tcode::rtti* rtti( void ){				                \
		return &class_name::s_rtti;							                \
	}														                \
	template < typename type >								                \
	bool is_kind_of() {										                \
...
#define IMPL_ROOT_RTTI( class_name )				const tcode::rtti class_name::s_rtti( #class_name , nullptr );
#define IMPL_CHILD_RTTI( class_name , parent_name ) const tcode::rtti class_name::s_rtti( #class_name , &parent_name::s_rtti );
...
TEST( rtti_test , no_virtual ) {
	...
	std::cout << tcode::rtti::get<no_virtal::parent>( root )->name() << std::endl;
	delete root;
